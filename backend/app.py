...
def get_habitability_ranking():
    ...
    - min_score: Minimum habitability score threshold (default: 0.0)
    ...
    try:
        ...
        if not os.path.exists(RANKING_PATH):
            return jsonify(
                {"status": "error", "message": f"Ranking data not found at {RANKING_PATH}"}
            ), 404
                    ...
                    planet_data.update(
                        {
                            ...
                            "discoverymethod": str(
                                meta_row.get("discoverymethod", "Unknown")
                            ),
                            # Additional planetary parameters
                            "pl_orbper": float(meta_row.get("pl_orbper", 0))
                            if pd.notna(meta_row.get("pl_orbper"))
                            else None,
                            "pl_orbsmax": float(meta_row.get("pl_orbsmax", 0))
                            if pd.notna(meta_row.get("pl_orbsmax"))
                            else None,
                            "pl_rade": float(meta_row.get("pl_rade", 0))
                            if pd.notna(meta_row.get("pl_rade"))
                            else None,
                            "pl_bmasse": float(meta_row.get("pl_bmasse", 0))
                            if pd.notna(meta_row.get("pl_bmasse"))
                            else None,
                            "pl_dens": float(meta_row.get("pl_dens", 0))
                            if pd.notna(meta_row.get("pl_dens"))
                            else None,
                            # Stellar parameters
                            "st_teff": float(meta_row.get("st_teff", 0))
                            if pd.notna(meta_row.get("st_teff"))
                            else None,
                            "st_rad": float(meta_row.get("st_rad", 0))
                            if pd.notna(meta_row.get("st_rad"))
                            else None,
                            "st_mass": float(meta_row.get("st_mass", 0))
                            if pd.notna(meta_row.get("st_mass"))
                            else None,
                            "st_age": float(meta_row.get("st_age", 0))
                            if pd.notna(meta_row.get("st_age"))
                            else None,
                            # System parameters
                            "sy_dist": float(meta_row.get("sy_dist", 0))
                            if pd.notna(meta_row.get("sy_dist"))
                            else None,
                        }
                    )
                except Exception as e:
        ...
        return jsonify(
            {"status": "error", "message": "Error retrieving ranking data", "error_details": str(e)}
        ), 500
...
def predict_batch():
    ...
    try:
        ...
        if not isinstance(planets, list):
            return jsonify(status="error", message="'planets' must be an array"), 400

        expected_features = get_feature_names()

        # 2) Helper to process one item
        def process_one(planet_data: dict, idx: int) -> dict:
            planet_id = planet_data.get("pl_name", f"Planet_{idx}")

            try:
                validation = validate_input_features(planet_data, expected_features)
                ...
                return {
                    ...
                    "status": "success",
                    "planet_id": planet_id,
                    "prediction_result": "Habitable" if pred == 1 else "Non-Habitable",
                    "confidence_score": float(np.round(prob, 4)),
                }
        ...
        # 3) Run batch (list-comprehension style)
        results = [
            process_one(planet_data, idx)
            for idx, planet_data in enumerate(planets)
            if isinstance(planet_data, dict)
        ]

        # Optional: handle invalid items (non-dict)
        for idx, planet_data in enumerate(planets):
            if not isinstance(planet_data, dict):
                results.insert(
                    idx,
                    {
                        "index": idx,
                        "status": "error",
                        "planet_id": f"Planet_{idx}",
                        "message": "Each item in 'planets' must be an object/dictionary",
                    },
                )
...
def model_info():
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "Error retrieving model information", "error_details": str(e)}
        ), 500
...
def get_planet_details(planet_index):
    ...
    try:
        if metadata_df is None:
            return jsonify({"status": "error", "message": "Metadata not available"}), 503
        ...
        return jsonify(
            {"status": "error", "message": "Error retrieving planet details", "error_details": str(e)}
        ), 500
